...
static long fence_ioctl(struct file *filp, unsigned int cmd,
                        unsigned long arg) {
  ...
  case (DMA_FENCE_IN_CMD):
    ...
    pr_info("Get in-fence from fd = %d\n", in_fence_fd);
    ...
    pr_info("Waiting in-fence to be signaled, process is blocking ...\n");
    dma_fence_wait(in_fence, true);
    pr_info("in-fence signaled, process exit\n");
  ...
  case (DMA_FENCE_OUT_CMD):
    ...
    pr_info("Created an out-fence fd = %d\n", out_fence_fd);
...
static int __init dma_fence_demo_init(void) { return misc_register(&mdev); }

static void __exit dma_fence_demo_unint(void) { misc_deregister(&mdev); }
